...
- **Test accounts in `vertex_dev`** (password `TestPassword123!` for all): `admin@test.local` (admin), `recruiter@test.local` (regional_recruiter, assigned to Kenya, supervised by supervisor@test.local), `supervisor@test.local` (country_supervisor), `new-recruiter@test.local` (regional_recruiter, created via the admin "Add Staff Member" flow to prove it end-to-end). Keep these — they aren't touched by the E2E suite's teardown (scoped to `e2e-*@test.local` only).
