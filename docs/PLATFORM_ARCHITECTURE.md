...
- `ScopeLevel` gained `inhouse` and `ReportStatus` gained `approved`/`escalated`/`closed` — In-House Supervisor can now submit its own portfolio-level report upward to Management/Director (`/inhouse/reports`, previously view/consolidate-only), auto-consolidated from its verified country report the same event-driven way country reports already auto-consolidate from recruiter reports (`reportConsolidation.ts`). Kept to the existing confirmed single-country In-House scoping (a portfolio of one country), not widened to multi-country, since that's a separate, unconfirmed scope change.
...
5. **Weekly/monthly country reports now submit themselves.** New `maybeAutoConsolidate` (`src/server/services/reportConsolidation.ts`), called from `PATCH /api/reports/[id]/verify` right after a recruiter report is marked verified: checks whether every recruiter report in the country for that exact `type`/`period_start`/`period_end` is now resolved (none left `submitted`), and if at least one was verified, auto-creates + submits the supervisor's own country report consolidating them — aggregating candidate counts and any challenges/performance-updates text pulled from the underlying recruiter reports' `content`. Deliberately doesn't wait for every recruiter under the supervisor to have filed something for that exact period (one on leave shouldn't block the country report indefinitely) — only for however many did submit to be resolved. Daily reports never auto-consolidate, matching last phase's "daily is good with the current structure." The old manual "Consolidate Report" flow stays on `/supervisor/reports` as a fallback (relabeled "Consolidate manually", with copy explaining when it's actually needed) for periods where auto-consolidation has nothing to work with — e.g. no recruiter reported at all.
