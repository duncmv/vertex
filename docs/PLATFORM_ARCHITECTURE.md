...
First phase of a planned role-by-role review (recruiter → country supervisor → in-house supervisor → director), driven by direct comments on the built portals. Built in an isolated git worktree (`recruiter-review-1`) against a dedicated scratch database (`vertex_dev_recruiter_review`/`vertex_test_recruiter_review`), since a second, unrelated session was concurrently editing the same shared working directory and dev database on its own branch — see the "Concurrent-session isolation" entry in CLAUDE.md's bug log for why. **The shared `vertex_dev`/`vertex_test` still need `prisma db push` + reseed run against them before this phase's schema (`RecruiterTarget`) is usable outside the scratch database** — deliberately not done automatically to avoid touching a database the other concurrent session depends on.
