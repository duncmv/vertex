...
### 1.7 Storage: private bucket + signed, time-limited URLs
**Decision:** flip the `vertex-documents` Supabase bucket to private. Replace `getPublicUrl` with `createSignedUrl(path, expiresIn)`, generated on-demand at view time (never cached long-term), gated by the same RBAC/scoping check that applies to the candidate record it belongs to.
**Why:** this directly closes FR-1.6, which is explicitly a Phase 1 **Must**. Passport scans currently sit behind a permanent, guessable-if-leaked public URL. This is the highest-priority fix in the entire plan and has no dependency on anything else, so it's sequenced first.
