...
// Phase 4's mobility lifecycle (SRS FR-4.1–4.3): approving an application
// auto-opens a Case, staff can advance its stage and issue a contract, and
// the candidate signs it themselves — the riskiest, most novel piece
// (a real in-house e-signature, not a stub) gets the end-to-end check.
test.describe("Mobility case lifecycle (Phase 4)", () => {
  test("approval opens a case, staff advance it and issue a contract, candidate signs it", async ({ page }) => {
    await test.step("admin approves the application, which auto-creates a case", async () => {
    ...
    await test.step("the case appears in the admin case list at its opening stage", async () => {
      await page.goto("/admin/cases");
      const caseCard = page.locator("a", { hasText: "E2E Case Candidate" });
      await expect(caseCard).toBeVisible({ timeout: 10_000 });
      await expect(caseCard.getByText(/Stage 1 of 11: Application/)).toBeVisible();
      await caseCard.click();
      await page.waitForURL(/\/admin\/cases\/.+/);
    });

    await test.step("admin advances the case to Verification with a deadline", async () => {
      ...
      await expect(page.getByText(/Stage 2 of 11: Verification/)).toBeVisible({ timeout: 10_000 });
      await expect(page.getByText("Docs look complete, moving to verification.")).toBeVisible();
    });

    await test.step("admin issues a contract", async () => {
    ...
    await test.step("the candidate signs their own contract from their dashboard", async () => {
      ...
      await expect(page.getByText("My Placement Case")).toBeVisible({ timeout: 10_000 });
      ...
      await expect(page.getByText(/Signed by E2E Case Candidate on/)).toBeVisible({ timeout: 10_000 });
    });
  });
});
