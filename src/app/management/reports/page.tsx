...
function ReportCard({
  ...
  return (
    <div className="card p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="font-medium text-midnight-900">{label}</span>
        ...
        {r.status === "submitted" && (
          <button onClick={onVerify} className="text-xs font-semibold text-emerald-600 hover:underline shrink-0">{verifyLabel}</button>
        )}
      </div>
      ...
      <button onClick={onToggleExpand} className="inline-flex items-center gap-1 text-xs text-gold-600 hover:underline mt-3">
        {expanded ? <CaretUp size={12} weight="bold" /> : <CaretDown size={12} weight="bold" />}
        Escalation trail ({r.child_reports.length} report{r.child_reports.length === 1 ? "" : "s"})
      </button>
      ...
      {expanded && (
        <div className="mt-3 bg-ivory-100 rounded-lg p-4 space-y-2">
          ...
          {r.child_reports.length === 0 && <p className="text-xs text-midnight-900/40">No source reports linked.</p>}
        </div>
      )}
...
export default function ManagementReportsPage() {
  ...
  const portfolioReports = reports.filter((r) => r.scope_level === "inhouse");
  const { page, setPage, totalPages, paged, total, pageSize } = usePagination(countryReports);
  const portfolioPagination = usePagination(portfolioReports);
  ...
  return (
    <PortalShell roleLabel="Management" navItems={MANAGEMENT_NAV_ITEMS}>
      ...
      <p className="text-midnight-900/55 font-light mb-8 max-w-2xl">
        In-House Supervisors&rsquo; portfolio reports and the underlying country reports they consolidate, with
        the full escalation trail from recruiter to country to here — approve to close the loop, or return for
        correction.
      </p>
        ...
        <div className="space-y-10">
          <section>
            <h2 className="text-sm font-semibold text-midnight-900/70 uppercase tracking-wider mb-4">
              Portfolio reports (In-House → Management)
            </h2>
            {portfolioReports.length === 0 ? (
              ...
              <div className="space-y-3">
                {portfolioPagination.paged.map((r) => (
                  <ReportCard
                    key={r.id}
                    r={r}
                    label={r.submitter.full_name}
                ...
                <Pagination
                  page={portfolioPagination.page}
                  totalPages={portfolioPagination.totalPages}
                  onPageChange={portfolioPagination.setPage}
                  total={portfolioPagination.total}
                  pageSize={portfolioPagination.pageSize}
                />
              </div>
            )}
          </section>
          ...
          <section>
            <h2 className="text-sm font-semibold text-midnight-900/70 uppercase tracking-wider mb-4">Country reports</h2>
